...
class Download_Network_Data_To_Local
{
    ...
    public function start()
    {
        if (empty($this->http_url) || empty($this->local_path)) {
            throw new \InvalidArgumentException('错误:必备参数不能为空,'
                . '请设置需要访问的网络地址与本地保存地址'
                . '-----or-----'
                . 'Error: The required parameters cannot be empty, '
                . 'please set the network address to be accessed and the local save address');
        }
        $this->http();
        if (isset($this->http_ret_info_res['http_code']) && $this->http_ret_info_res['http_code'] != 200) {
            throw new \InvalidArgumentException('错误:无法访问网络,请确认网络正常!'
                . '-----or-----'
                . 'Error: Unable to access the network, please confirm that the network is normal!');
        }
        ...
        if (\is_readable($tmp_filename)) {
            throw new \InvalidArgumentException('错误:文件已存在!'
                . '-----or-----'
        ...
        throw new \InvalidArgumentException('错误:文件无法保存!'
            . '-----or-----'
